...
namespace Spg.SpengerShop.ApplicationTest
{
    public class ProductServiceTest //: IClassFixture<ProductServiceFixture>
    {
        ...
        [Fact]
        public void Create_GetAll_Success_Test()
        {
            using (SpengerShopContext db = new SpengerShopContext(DatabaseUtilities.GetDbOptions()))
            {
                // Arrange
                ...
                // Act
                IReadOnlyProductService unitToTest = new ProductService(
                    _productRepositoryBase,
                    _readonlyProductRepository,
                    _readonlyCategoryRepository,
                    _dateTimeService
                );

                IReadOnlyProductService actual = unitToTest
                    .Load();
                    //.UseFilterByName("Mobil")
                    //.GetDataPaged(1, 2);

                // Assert
                Assert.Equal(2, unitToTest.Products.Count());
            }
        }
    }
}